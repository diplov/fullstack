...
exports.addUser = async (req, res) => {
  ...
  if (user) {
    return res.status(400).json({
      error: "Email already exists. please login 0r try different email",
    });
  }
  // create user
...
exports.verifyEmail = async (req, res) => {
  ...
  return res
    .status(200)
    .json({ message: "User verified Successfully.login to continue" });
};
// forget password
...
exports.signin = async (req, res) => {
  ...
  if (!user.isVerified) {
    return res
      .status(400)
      .json({ error: "user not verified.verified to continue" });
  }
  // create login token
...
exports.resendverification = async (req, res) => {
  ...
  return res
    .status(200)
    .json({ message: "Verification link is send to email.please verified" });
};
